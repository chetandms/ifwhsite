	...
	<?php if ( $this->options->get( 'header_menu' ) ): ?>
		...
		<amp-accordion>
			<section class="<?php echo $css_class ?>-section">
				<h4 class="accordion-header hamburger <?php echo $css_class ?>">
                    <?php if ( $this->options->get( 'header_menu_button' ) == 'text' ){ _e( 'Menu', 'amphtml' ); } ?>
				</h4>
		...
		<?php else: ?>
			<div class="accordion-header hamburger <?php echo $css_class ?>-section">
				<button class="amp-menu-sidebar <?php echo $css_class ?>" on='tap:amp-sidebar.toggle'><?php if ( $this->options->get( 'header_menu_button' ) == 'text' ){ _e( 'Menu', 'amphtml' ); } ?></button>
			</div>
		<?php endif; ?>
	<?php endif; ?>
</div>